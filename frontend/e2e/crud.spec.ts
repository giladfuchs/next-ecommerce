...
test("view first order details page", async ({ page }) => {
  ...
  await navigateToAdminModel(page, TestModelType.order);
...
test("navigate through admin pages and check tables render", async ({
  ...
  await loginToAdmin(page);
  for (const model of Object.values(TestModelType)) {
    await navigateToAdminModel(page, model as TestModelType);
  }
});

test("delete first product/category", async ({ page }) => {
  await loginToAdmin(page);
  await deleteFirstRowFromModel(page, TestModelType.product);
  await deleteFirstRowFromModel(page, TestModelType.category);
});

test("edit first category", async ({ page }) => {
  await openFirstEditForm(page, TestModelType.category);
  await fillBasicCategoryForm(page);

  await submitForm(page, TestModelType.category);
});

test("add category and verify count increase", async ({ page }) => {
  await loginToAdmin(page);
  await navigateToAdminModel(page, TestModelType.category);
  const rowCountBefore = await getRowCount(page);
  await page.getByTestId("add-category-button").click();
  await expect(page).toHaveURL(
    `${TEST_BASE_URL}/admin/form/${TestModelType.category}/add`,
  );
...
test("edit first product", async ({ page }) => {
  await openFirstEditForm(page, TestModelType.product);

  await fillBasicProductForm(page);
  await submitForm(page, TestModelType.product);
});

test("add product and verify count increase", async ({ page }) => {
  await loginToAdmin(page);
  await navigateToAdminModel(page, TestModelType.product);
  const rowCountBefore = await getRowCount(page);
  await page.getByTestId("add-product-button").click();
  await expect(page).toHaveURL(
    `${TEST_BASE_URL}/admin/form/${TestModelType.product}/add`,
  );
