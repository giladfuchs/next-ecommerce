...
router.get(
  ...
  withErrorHandler(async (req: Request, res: Response) => {
    const order = await AuthController.getOrder(req.params.id as string);
    res.json(order);
  }),
);
...
router.post(
  ...
  withErrorHandler(async (req: Request, res: Response) => {
    ...
    const result = await AuthController.saveProduct(add_or_id as string, req.body);
...
router.post(
  ...
  withErrorHandler(async (req: Request, res: Response) => {
    const { add_or_id } = req.params;
    const result = await AuthController.saveCategory(add_or_id as string, req.body);
    return res.status(add_or_id === "add" ? 201 : 200).json(result);
  }),
);
